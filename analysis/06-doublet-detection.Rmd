...
```{r}
pacman::p_load(devtools, here, readr, scater, scran)
```

Read experiment data:

```{r}
rds <- here("data/05-clustering.rds")
...
```{r eval = run}
dbl <- doubletCluster(alt, alt$cluster)
```
...
```{r eval = run}
res <- findMarkers(alt, alt$cluster, direction = "up")
...
for (n in names(sig)) {
  ...
  plotHeatmap(alt, order_columns_by = "cluster", features = ids, center = TRUE, symmetric = TRUE)
